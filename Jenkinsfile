            ...
            stages {
                stage("Build staging image") {
                    steps {
                        ...
                        withCredentials([string(credentialsId: "discord-vchain", variable: "WEBHOOK_URL")]) {
                            discordSend(
                                description: "\n \
                        ...
                        withCredentials([usernamePassword(credentialsId: 'docker-devopstovchain', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME'),
                                         file(credentialsId: 'env-vchain-front-end-staging', variable: 'ENV_VCHAIN_FRONT_END_STAGING')]) {
                            writeFile(file: ".env", text: readFile("${ENV_VCHAIN_FRONT_END_STAGING}"), encoding: "UTF-8")
            ...
            stages {
                stage("Build production image") {
                    steps {
                        ...
                        withCredentials([usernamePassword(credentialsId: 'docker-devopstovchain', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME'),
                                         file(credentialsId: 'env-vchain-front-end-production', variable: 'ENV_VCHAIN_FRONT_END_PRODUCTION')]) {
                            writeFile(file: ".env", text: readFile("${ENV_VCHAIN_FRONT_END_PRODUCTION}"), encoding: "UTF-8")
                            sh "docker build -t ${env.IMAGE_NAME} . && \
                                docker login -u ${DOCKER_USERNAME} -p ${DOCKER_PASSWORD} ${REGISTRY} && \
                                docker push ${env.IMAGE_NAME} && \
                                docker logout"
